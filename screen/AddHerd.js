...
const AddHerd = ({ navigation, route }) => {
  const goBack = () => {
    navigation.goBack();
  };
  ...
  const handleSave = async () => {
    ...
    if (
      ...
      // navigation.navigate("ViewPaddock", { cattleClass: herdData.cattleClass });
      try {
        let herds = await AsyncStorage.getItem("herds");
        herds = herds ? JSON.parse(herds) : [];
        herds = herds.map((herd) => ({
          ...herd,
          dateEntered: new Date(herd.dateEntered),
          exitDate: new Date(herd.exitDate),
        }));
        ...
        // ) {
        //   Alert.alert("Error", "A paddock with this number already exists.");
        //   return;
        // }

        herds.push(newHerdData);
        await AsyncStorage.setItem("herds", JSON.stringify(herds));

        Alert.alert("Success", "Herd added to the database successfully.", [
        ...
        console.error(error);
        Alert.alert("Error", "An error occurred while saving the herd data.");
      }
    }
  };
